...
export function Board({ attributes, auto, onSubmit }: BoardProps) {
    ...
    useEffect(() => {
        ...
        const timers = ATTRIBUTE_KEYS.map((key, i) =>
            setTimeout(
                () => setSel((s) => ({ ...s, [key]: autoSel[key] })),
                AUTO_PICK_MS[i],
            ),
        );
        timers.push(
            setTimeout(() => submitRef.current(autoSel), AUTO_SUBMIT_MS),
        );
    ...
    useEffect(() => {
        ...
        for (const key of ATTRIBUTE_KEYS)
            for (const value of attributes[key])
                if (!NOTES[value])
                    console.warn(`board: no position for attribute "${value}"`);
    }, [attributes]);
                ...
                {complete && !auto && (
                    ...
                    >
                        <span
                            ...
                            style={{ background: SOLVE_NOTE.pin }}
                        />
